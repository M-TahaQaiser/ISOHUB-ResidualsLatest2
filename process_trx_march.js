// Process TRX March 2025 Excel Data
import XLSX from 'xlsx';
import fs from 'fs';

console.log('📊 PROCESSING TRX MARCH 2025 EXCEL');
console.log('='.repeat(80));

try {
  // Read the Excel file
  const workbook = XLSX.readFile('attached_assets/TRX march 2025 - Raw_1753237886335.xlsx');
  ...
  console.log(`\n📊 TRX MARCH 2025 PROCESSING RESULTS:`);
  ...
  // Process each record looking for revenue fields (based on TRX April structure)
  data.forEach((record, index) => {
    try {
      // Look for potential revenue fields (Agent Residual from TRX April)
      const possibleRevenueFields = Object.keys(record).filter(key => 
        key.toLowerCase().includes('residual') ||
      ...
      // Find revenue (prioritize Agent Residual pattern from TRX)
      ...
      // Find transactions/volume
      for (const field of possibleTransactionFields) {
        const value = parseFloat(record[field]);
        if (!isNaN(value) && value > 0) {
          if (field.toLowerCase().includes('sales') || field.toLowerCase().includes('volume')) {
            volume = value;
          } else {
            transactions = value;
          }
  ...
  if (merchantData.length > 0) {
    console.log(`\n🏆 TOP TRX MERCHANTS (MARCH 2025):`);
    const topMerchants = merchantData
      .sort((a, b) => b.net - a.net)
      .slice(0, 10);
      
    topMerchants.forEach((merchant, index) => {
      console.log(`${index + 1}. ${merchant.name}: $${merchant.net.toFixed(2)} (Volume: $${merchant.volume.toFixed(2)})`);
    });
  }
  ...
  if (data.length > 0) {
    ...
    console.log('Revenue-like fields:', Object.keys(sampleRecord).filter(k => 
      k.toLowerCase().includes('residual') || k.toLowerCase().includes('agent') || k.toLowerCase().includes('amount')
    ));
    ...
    console.log('Volume-like fields:', Object.keys(sampleRecord).filter(k => 
      k.toLowerCase().includes('sales') || k.toLowerCase().includes('volume')
    ));
  }
  
  console.log(`\n✅ TRX MARCH 2025 PROCESSING COMPLETE`);
  console.log(`${validMerchants} merchants processed successfully`);
  
} catch (error) {
  console.error('Error processing TRX March data:', error);
  process.exit(1);
}
