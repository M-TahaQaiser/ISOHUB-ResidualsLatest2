...
export default function Login({ setUsername, setAuthToken, setOrganization }: LoginProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Attempt login with migrated MongoDB data
      const { token } = await login(localUsername, password);
      
      // Decode the token to get the organizationID
      const decodedToken = decodeToken(token);
      const organizationID = decodedToken.organization || decodedToken.organizationID || 'org-86f76df1';
      
      // Store authentication data
      setStoredAuth(token, localUsername, organizationID);
  ...
  return (
    <div className="min-h-screen bg-black flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-zinc-900 border-yellow-400/20">
        ...
        <CardContent>
          {/* SSO Login Options */}
          {(ssoStatus?.google?.configured || ssoStatus?.microsoft?.configured) && (
            <div className="mb-6">
              <div className="space-y-3">
                {ssoStatus?.google?.configured && (
                  <Button
                    ...
                    className="w-full bg-white hover:bg-gray-100 text-gray-800 border border-gray-300 py-3 font-medium flex items-center justify-center gap-3"
                ...
                {ssoStatus?.microsoft?.configured && (
                  <Button
                    ...
                    className="w-full bg-[#2F2F2F] hover:bg-[#1F1F1F] text-white py-3 font-medium flex items-center justify-center gap-3"
              ...
              <div className="relative my-6">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-zinc-900 text-gray-400">or continue with email</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mt-6 p-4 bg-zinc-800 rounded-lg border border-yellow-400/30">
            <h3 className="text-yellow-400 font-semibold mb-3 text-sm">Quick Login Credentials:</h3>
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between items-center p-2 bg-zinc-700 rounded hover:bg-zinc-600 cursor-pointer transition-colors"
                   onClick={() => { setLocalUsername('cburnell24'); setPassword('password'); }}>
                <span className="text-gray-300">Cody Burnell (Agent)</span>
                <span className="text-yellow-400 font-mono">cburnell24 / password</span>
              </div>
              <div className="flex justify-between items-center p-2 bg-zinc-700 rounded hover:bg-zinc-600 cursor-pointer transition-colors"
                   onClick={() => { setLocalUsername('jkeanffd'); setPassword('password'); }}>
                <span className="text-gray-300">James Kean (Agent)</span>
                <span className="text-yellow-400 font-mono">jkeanffd / password</span>
              </div>
